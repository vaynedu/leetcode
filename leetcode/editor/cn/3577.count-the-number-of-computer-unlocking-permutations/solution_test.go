...
complexity[i]。
 ...
 可以使用编号为 j 的计算机的密码解锁编号为 i 的计算机，其中 j 是任何小于 i 的整数，且满足 complexity[j] < complexity[
i]（即 j < i 并且 complexity[j] < complexity[i]）。
...
func TestCountTheNumberOfComputerUnlockingPermutations(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := countPermutations(tt.complexity)
			if result != tt.expected {
				t.Errorf("countPermutations(%v) = %d; expected %d", tt.complexity, result, tt.expected)
			}
		})
	}
}
